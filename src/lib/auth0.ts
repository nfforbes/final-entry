...
export const auth0 = new Auth0Client({
  ...
  onCallback: async (error, ctx, session) => {
    if (error) {
      ...
      return NextResponse.redirect(new URL('/auth/login?error=callback', ctx.appBaseUrl || 'http://localhost:3000'));
    }
    ...
    if (session) {
      ...
      if (user && user.sub && user.email) {
        ...
        try {
          const syncUrl = new URL('/api/users/sync', ctx.appBaseUrl || 'http://localhost:3000');
          ...
          if (syncData.user?.role === 'admin') {
            ...
            return NextResponse.redirect(new URL('/admin/dashboard', ctx.appBaseUrl || 'http://localhost:3000'));
          }

          if (syncData.user?.role === 'technician') {
            console.log(`[Auth0] Technician detected, redirecting to /technician/dashboard`);
            return NextResponse.redirect(new URL('/technician/dashboard', ctx.appBaseUrl || 'http://localhost:3000'));
          }
        } catch (syncError) {
