...
export async function POST(req: NextRequest) {
  ...
  if (!isAdmin && session?.user?.sub) {
    ...
    if (user?.role === 'admin') {
      isAdmin = true;
      console.log(`[Auth Fallback] Admin role verified via DB for Microsoft Test: ${session.user.email}`);
    }
  }
  ...
  try {
    await sendEmail({
      to,
      subject: 'Final Entry - Microsoft Integration Test',
      htmlBody: `
        <div style="font-family: sans-serif; padding: 20px; border: 1px solid #c6f135; border-radius: 8px;">
          ...
          <p>Your Microsoft 365 account is now successfully linked to the system.</p>
